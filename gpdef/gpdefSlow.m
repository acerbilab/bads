function gpstruct = gpdefSlow(covtype,covextras,D,gplik,optimState,options,gpstruct)
%GPDEFSLOW Define Gaussian Process (GP) structure for BADS with slow-varying features.
...
maxcovlen = 100;        % Maximum covariance length (normalized)
...
if isempty(gpstruct)
    ...
    covrange = min(maxcovlen, 10*covrange);
    for i = 1:gpstruct.ncovlen
    ...
    % Prior over length scales (we favor slower-changing priors)
    for i = (1:gpstruct.ncovlen) + gpstruct.ncovoffset
        gpstruct.prior.cov{i} = {@priorLaplace,log(maxcovlen),1^2};
    end        
